...
@section('content')
    <div data-reveal class="flex items-center justify-between gap-3">
        <div class="space-y-1">
            <h2 class="text-xl font-extrabold text-slate-900">Blog</h2>
            <p class="text-sm text-slate-500">Artikel teknis yang tampil di section Blog halaman publik.</p>
        </div>
        <a href="{{ route('admin.blog.create') }}" class="shrink-0 inline-flex items-center gap-2 px-4 py-2.5 bg-slate-900 hover:bg-indigo-600 text-white text-xs font-bold rounded-xl transition-colors">
    ...
    {{-- Search & Filter --}}
    <form data-reveal method="GET" action="{{ route('admin.blog') }}" class="bg-white/60 backdrop-blur-xl rounded-3xl p-4 sm:p-5 border border-white/80 shadow-2xs flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
            <x-icon name="search" class="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
            <input type="text" name="search" value="{{ $search }}" placeholder="Cari judul artikel..." class="w-full pl-10 pr-4 py-2.5 bg-white/70 backdrop-blur-md rounded-xl border border-white/90 text-sm text-slate-800 focus:bg-white focus:outline-indigo-500 shadow-2xs transition-colors" />
        </div>
        ...
        @if ($search || $categoryFilter)
            <a href="{{ route('admin.blog') }}" class="px-5 py-2.5 bg-white/70 hover:bg-white text-slate-600 text-sm font-bold rounded-xl border border-white/90 transition-colors text-center">Reset</a>
        @endif
    </form>
    ...
    >
        <div class="px-5 sm:px-6 py-4 border-b border-slate-200/70">
            <h3 class="text-sm font-extrabold text-slate-900">Daftar Artikel ({{ $posts->total() }})</h3>
        </div>

        @if ($posts->isEmpty())
            <div class="py-16 text-center text-sm text-slate-400">Tidak ada artikel yang cocok dengan filter ini.</div>
        @else
                    ...
                    <li class="flex items-center justify-between gap-4 px-5 sm:px-6 py-4">
                        <div class="flex items-center gap-3.5 min-w-0">
                            <div class="flex flex-col shrink-0">
                                <form method="POST" action="{{ route('admin.blog.move', $post) }}">
                                    @csrf @method('PATCH')
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="text-xs text-slate-500 truncate">{{ $post->category }} &bull; {{ $post->published_at }} &bull; {{ count($post->sections ?? []) }} bagian</div>
                            </div>
                        </div>

                        <div class="flex items-center gap-1.5 shrink-0">
                            <a href="{{ route('admin.blog.edit', $post) }}" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50/80 rounded-lg transition-colors" title="Edit">
                                <x-icon name="file-code" class="w-4 h-4" />
                            </a>
                            <button
                                type="button"
                                @click="modalOpen = true; deleteUrl = '{{ route('admin.blog.destroy', $post) }}'; deleteLabel = '{{ addslashes($post->title) }}'"
        ...
        >
            ...
            >
                ...
                <div class="space-y-1">
                    <h4 class="font-bold text-slate-900 text-sm">Hapus artikel ini?</h4>
